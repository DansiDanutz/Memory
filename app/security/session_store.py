...
def cleanup_expired():
    """Clean up expired sessions (for memory store only)"""
    if isinstance(_store, _Mem):
        now = time.time()
        expired = [k for k, v in _store.m.items() if v < now]
        for k in expired:
            _store.m.pop(k, None)

def session_time_remaining(phone: str) -> int:
    """Get remaining time in minutes for session"""
    if isinstance(_store, _Mem):
        exp = _store.m.get(phone)
        if exp:
            remaining = exp - time.time()
            if remaining > 0:
                return int(remaining / 60) + 1
    elif isinstance(_store, _Redis):
        ttl = _store.r.ttl(_PREFIX + phone)
        if ttl > 0:
            return int(ttl / 60) + 1
    return 0
