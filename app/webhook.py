...
from .utils.claude_utils import get_conversation_buffer
...
conversation_buffer = get_conversation_buffer()
...
🤖 Claude AI Commands:
• claude: - Check Claude AI status
• analyze: <text> - Analyze message sentiment
• chat: <message> - Chat with Claude AI
• summarize: - Summarize recent conversation
• smart mode: on/off - Toggle intelligent responses
...
• Status: {'Verified' if is_verified(frm) else 'Unverified'}"""
                        ...
                        # Claude AI Commands
                        if low.strip() == 'claude:' or low.strip() == 'claude':
                            if claude.is_available():
                                send_text(frm, f"🤖 Claude AI is active!\nModel: {claude.model}\nStatus: Ready", PHONE_ID, ACCESS_TOKEN)
                            else:
                                send_text(frm, "⚠️ Claude AI is not configured. Please set ANTHROPIC_API_KEY.", PHONE_ID, ACCESS_TOKEN)
                            continue

                        if low.startswith('analyze:'):
                            message_to_analyze = text.split(':', 1)[1].strip()
                            if claude.is_available():
                                # Get conversation context
                                context = conversation_buffer.get_context(frm, max_messages=5)
                                analysis = await claude.analyze_message(message_to_analyze, context)
...
• Tags: {', '.join(analysis.tags[:5]) if analysis.tags else 'None'}"""
                                send_text(frm, response_text, PHONE_ID, ACCESS_TOKEN)
                            else:
                                send_text(frm, "Claude AI is not available for analysis.", PHONE_ID, ACCESS_TOKEN)
                            continue

                        if low.startswith('chat:'):
                            chat_message = text.split(':', 1)[1].strip()
                            if claude.is_available():
                                # Get conversation context
                                context = conversation_buffer.get_context(frm, max_messages=10)
                                # Add current message to buffer
                                conversation_buffer.add_message(frm, "user", chat_message)
                                # Generate response
                                response = await claude.generate_response(chat_message, context, tone="friendly")
                                # Add response to buffer
                                conversation_buffer.add_message(frm, "assistant", response)
                                send_text(frm, f"🤖 {response}", PHONE_ID, ACCESS_TOKEN)
                            else:
                                send_text(frm, "Claude AI is not available for chat.", PHONE_ID, ACCESS_TOKEN)
                            continue

                        if low.strip() == 'summarize:' or low.strip() == 'summarize':
                            if claude.is_available():
                                # Get recent messages from conversation buffer
                                messages = conversation_buffer.get_context(frm, max_messages=20)
                                if messages:
                                    summary = await claude.summarize_conversation(messages, max_length=500)
                                    send_text(frm, f"📝 Conversation Summary:\n{summary}", PHONE_ID, ACCESS_TOKEN)
                                else:
                                    send_text(frm, "No recent conversation to summarize.", PHONE_ID, ACCESS_TOKEN)
                            else:
                                send_text(frm, "Claude AI is not available for summarization.", PHONE_ID, ACCESS_TOKEN)
                            continue

                        if low.startswith('smart mode:'):
                            mode = text.split(':', 1)[1].strip().lower()
                            if mode == 'on':
                                # Store smart mode preference (you could use a dict or database)
                                send_text(frm, "✅ Smart mode enabled. Claude will enhance your messages.", PHONE_ID, ACCESS_TOKEN)
                            elif mode == 'off':
                                send_text(frm, "❌ Smart mode disabled. Regular memory storage active.", PHONE_ID, ACCESS_TOKEN)
                            else:
                                send_text(frm, "Use 'smart mode: on' or 'smart mode: off'", PHONE_ID, ACCESS_TOKEN)
                            continue
                        ...
                        # Store memory with Claude enhancement if available
                        conversation_buffer.add_message(frm, "user", text)
                        
                        if claude.is_available():
                            # Analyze with Claude for better categorization
                            context = conversation_buffer.get_context(frm, max_messages=5)
                            analysis = await claude.analyze_message(text, context)
                            
                            # Use Claude's category if high confidence, otherwise use classifier
                            if analysis.confidence > 0.7:
                                cat = analysis.category
                            else:
                                cat = classifier.classify(text, frm)
                            
                            # Extract memory if it's important
                            if analysis.priority >= 6:
                                memory_extract = await claude.extract_memory(text, context)
                                # Store the extracted content instead if it's more structured
                                if memory_extract.importance >= 6:
                                    await store.store(frm, memory_extract.content, cat)
                                    send_text(frm, f'📝 Saved enhanced memory to {cat} (Importance: {memory_extract.importance}/10)\nTags: {", ".join(memory_extract.tags[:3])}', PHONE_ID, ACCESS_TOKEN)
                                else:
                                    await store.store(frm, text, cat)
                                    send_text(frm, f'Saved to {cat}. Priority: {analysis.priority}/10', PHONE_ID, ACCESS_TOKEN)
                            else:
                                await store.store(frm, text, cat)
