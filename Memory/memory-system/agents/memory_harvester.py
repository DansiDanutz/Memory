...
class MemoryHarvesterAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "memory_harvester", config: Dict[str, Any] = None):
        capabilities = [
            AgentCapability.DATA_HARVESTING,
    ...
    async def _initialize_components(self):
        """Initialize agent-specific components (required by BaseAgent)"""
        await self._initialize_content_processors()
        await self._initialize_quality_validators()
        await self._initialize_metadata_extractors()
        await self.duplicate_detector.initialize()
    ...
    async def _handle_message(self, message) -> Optional[Dict[str, Any]]:
        """Handle incoming message (required by BaseAgent)"""
        try:
            if hasattr(message, 'content'):
                raw_input = RawMemoryInput(
                    content=message.content,
                    source_type=SourceType.CHAT_MESSAGE,
                    source_metadata={'channel': 'agent_message'},
                    user_id=getattr(message, 'user_id', 'default_user')
                )
                processed = await self.process_memory(raw_input)
                return {
                    'success': True,
